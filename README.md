...
界面基于 Python 和 pywebview，支持设备信息、数字 IO、ADC、PWM、I²C 扫描和读写、SPI 全双工、UART 透传配置、通用串口终端（文本/HEX、时间戳、行尾、日志保存）以及最多 4 路轮询波形和 CSV 导出。“目标设备调试”页把 I²C/SPI 事务和 UART 调试入口集中在一起；I²C/SPI 有独立的带时间戳收发记录和 TXT 导出。启用总线时，上位机自动将选中的输入或 ADC 通道切回浮空，不抢占输出/PWM；若启用失败，会尽量恢复原模式。串口终端仍可连接任意系统串口；UART 一键调试会按 USB 接口位置匹配同一开发板的 CDC1，而不猜测 COM 号。连接旧固件时，原有功能仍可使用，新总线功能按能力位提示不可用。
...
电脑通过原生 USB 连接 ESP32-S3；ESP32-S3 的 GPIO 再连接目标设备。上位机选择控制口并点击“连接并读取”，进入“目标设备调试”页。先在“数字 IO”页核对通道与实际 GPIO 的映射。目标设备与开发板必须共地，GPIO 只允许 3.3 V 逻辑；不同电压域需电平转换，不要直接接 5 V。

| 协议 | 接线与上位机操作 | 数据返回方式 |
|---|---|---|
| I²C 主机 | SDA、SCL、GND；配置引脚和频率，扫描 7 位地址，再指定目标地址与写入 HEX/读取字节数；总线需合适上拉 | 每次点击“执行写 / 读 / 写后读”后，在当前页显示和记录结果 |
| SPI 主机 | SCLK、MOSI、MISO、CS、GND；配置模式和频率，输入发送 HEX；CS 可配置为自动片选 | 每次点击“全双工传输”后返回等长接收字节 |
| UART | 本板 TX 接目标 RX、本板 RX 接目标 TX，另接 GND；点击“配置并打开 UART 调试终端” | 上位机自动打开同一板子的第二个 USB 虚拟串口，持续双向收发文本或 HEX |

这里的“中转”是由电脑指定事务、板子在外部引脚上执行、再把结果送回电脑。I²C/SPI 需要地址、片选和事务边界，因此通过控制口的协议命令传输，不是把第二个 COM 口的原始字节直接当作 I²C/SPI 信号。当前固件是主动主机调试器，不是并联监听现有总线的无侵入嗅探器或高速逻辑分析仪。
...
UART 配置成功后可用上位机“UART 串口终端”打开 CDC1 端口，或直接在“目标设备调试”页使用一键入口。示例通道须先处于浮空输入状态；I²C 需外部上拉电阻。所有信号都是 3.3 V。波形图是软件轮询趋势图，不是高速逻辑分析仪。
